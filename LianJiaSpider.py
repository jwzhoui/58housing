...
def gen_xiaoqu_insert_command(info_dict):
    ...
    for il in info_list:
        if il in info_dict:
            t.append(info_dict[il].encode("utf-8"))
        else:
            t.append(''.encode("utf-8"))
    t=tuple(t)
    ss=(r"insert into xiaoqu SET name=%s,regionb=%s,regions=%s,style=%s,year=%s;", t)
    return ss
...
def xiaoqu_spider(db_xq,url_page=u"http://bj.lianjia.com/xiaoqu/pg1rs%E6%98%8C%E5%B9%B3/"):
    ...
    xiaoqu_list=soup.findAll('li',{'class':['xiaoquListItem','clear'],'data-el':'xiaoqu'})
    for xq in xiaoqu_list:
        info_dict={}
        info_dict.update({u'小区名称':xq.select('.title a ')[0].string})
        content=xq.find('div',{'class':'positionInfo'})
        # info=re.match(r'.+',content)
        infos=xq.find('div', {'class': 'positionInfo'})
        d=infos.find('a', {'class': 'district'}).string
        x=infos.find('a',{'class':'bizcircle'}).string
        h=infos.text.replace(' ', '').replace('\n','').split(r'/')
        info_dict.update({u'大区域':d})
        info_dict.update({u'小区域':x})
        info_dict.update({u'小区户型':h[len(h)-2]})
        info_dict.update({u'建造时间':h[len(h)-1][:5]})
...
def do_xiaoqu_spider(db_xq,region=u"昌平"):
    ...
    for i in range(total_pages):
        ...
        ww=xiaoqu_spider(db_xq,url_page)
...
if __name__=="__main__":
    command="create table if not exists xiaoqu (id integer auto_increment primary key UNIQUE,name text, regionb TEXT, regions TEXT, style TEXT, year TEXT)"
    db_xq=SQLiteWraper('lianjia-xq.db',command)

    command="create table if not exists chengjiao (href varchar(64) primary key UNIQUE, name TEXT, style TEXT, area TEXT, orientation TEXT, floor TEXT, year TEXT, sign_time TEXT, unit_price TEXT, total_price TEXT,fangchan_class TEXT, school TEXT, subway TEXT)"
